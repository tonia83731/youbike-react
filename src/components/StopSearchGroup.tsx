...
const StopSearchGroup = () => {
    const {districtToggle, handleDistrictToggle} = useFilterContext()
    return <div className="grid grid-cols-[2fr_40px_40px] w-full gap-4 lg:grid-cols-[2fr_50px]">
        <StopSearch />
        <button 
            // onClick={handleDistrictToggle}
            className="font-bold text-xl flex justify-center items-center bg-gray_input text-dark/60 h-[40px] lg:h-[50px] rounded-full">
            <GiHumanTarget />
        </button>
        <button 
            onClick={handleDistrictToggle}
